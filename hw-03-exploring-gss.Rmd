...
***The possible responses are Does Not Apply, No, and Yes.***
```{r}
xtabs(~ harass5, gss16)
```
***96 people choose "Does not apply", 1136 people said "No", and 237 people said "Yes".***
...
***1136 / 237 = 4.79, 100 / 4.79 = 20.88. So, 20.88% of respondents have been harassed at their job.***
...
```{r}
gss16 %>%
  summarise(means = mean(email), medians = median(email))
```

    Is the mean or the median a better measure of the typical among of time Americans spend on email weekly? Why?
    
***The median is a better measure of the typical time on email weekly, because unlike the mean, the median does not take into account the lower or upper extremes of the spectrum. The median just looks at the middle of the data.***
...
snap_insta <- c("snapchat", "instagram") %>%
  mutate(
    snap_insta = if_else(snapchat == "yes", "yes", "No")
...
***The various responses are working fulltime, working parttime, retired, keeping house, temp not working, unempl laid off, school, and other.***

```{r}
xtabs(~ wrkstat, gss16)
```

***284 people were keeping house, 89 people were doing something else, 574 people were retired, 76 people were attending school, 57 people were temporarily unemployed, 118 people were laid off, 1321 people were laid off, and 345 people were working part time.***
...
Recoded <- gss16 %>%
  mutate(
   polviews = if_else(polviews == "Extremely liberal", "Liberal", "conservative")
  ) %>%  mutate(
   polviews = if_else(polviews == "Slightly liberal", "Liberal", "conservative")
  ) %>%
   mutate(
   polviews = if_else(polviews == "Liberal", "Liberal", "conservative")
   ) %>%   mutate(
   polviews = if_else(polviews == "Extrmly conservative", "Conservative", "Liberal")
  ) %>%  mutate(
   polviews = if_else(polviews == "Slightly Conservative", "Conversative", "Liberal")
  ) %>%
   mutate(
   polviews = if_else(polviews == "Conservative", "Conservative", "Liberal")
   ) 
```
